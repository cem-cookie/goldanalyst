...
Comparison:
1. GPT-4o-mini (base model)
2. GPT-4o-mini (Fine-tuned model)

Requirements:
- Use GPT4oAgent
- Call decide() -> execute()
- Read data/news/{date}/market_analysis.txt
- Read data/gold_history.csv
...
def load_prices(csv_path):
    ...
    """
    # 1. Read first line to check if it has header
    ...
    # If first column is date / Date / DAY etc, has header
    has_header = first_col in ["date", "day", "time"]

    if has_header:
        df = pd.read_csv(csv_path)
    else:
        # No header: treat as two columns
        df = pd.read_csv(csv_path, header=None, names=["date", "price"])

    # Normalize date column
    if "date" not in df.columns:
        # Find column with 'date' in name
        date_col = None
        for c in df.columns:
            if "date" in c.lower():
                date_col = c
    ...
    if "price" not in df.columns:
        price_col = None
        # Try typical price column names
        for c in df.columns:
            cl = c.lower()
            if cl in ["price", "close", "gold", "xauusd", "xau", "value"]:
                price_col = c
                break
        # If not found, use non-date second column
    ...
    # Only keep YYYY-MM-DD format rows
...
def plot_agent_comparison(agents, price_df):
    """Compare two agent equity curves + gold price."""
    fig, ax1 = plt.subplots(figsize=(13, 7))

    # Gold price curve (right axis)
    ax2 = ax1.twinx()
    ax2.plot(price_df["date"], price_df["price"], color="#DAA520", linewidth=2.5, label="Gold Price")

    # Agent equity (left axis)
...
def run():
    ...
    # Filter price_df to only dates with analysis
    ...
    # ------------------------------------------------
    # Initialize two models (base vs finetuned)
    # ------------------------------------------------
    ...
    for i, row in price_df.iterrows():
        ...
        for agent in agents:
            ...
            print(f"  {agent.name:<18} {last['action']:>5} {last['amount_oz']:>6.2f} oz "
                  f"{'Y' if last['executed'] else 'N'} | Eq: ${last['equity']:>10.0f}")
